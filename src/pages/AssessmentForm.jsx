...
export default function AssessmentForm({ theme, setTheme }){
  const { id } = useParams()
  const navigate = useNavigate()
  const [items, setItems] = useState([
    {label:'Keterampilan 1', value: 80},
    {label:'Keterampilan 2', value: 75},
    {label:'Keterampilan 3', value: 90},
  ])

  function handleChange(idx, val){
    const copy = [...items]
    copy[idx].value = Number(val)
    setItems(copy)
  }
  function submit(e){
    e.preventDefault()
    const avg = Math.round(items.reduce((s,i)=>s+i.value,0)/items.length * 100)/100
    localStorage.setItem('lastResult', JSON.stringify({id, avg, items}))
    navigate(`/result/${id}`)
  }
  ...
  return (
    <div className="min-h-screen p-6">
      ...
      <div className="mt-6 max-w-3xl mx-auto card">
        <h3 className="font-semibold text-lg">Form Penilaian - ID {id}</h3>
        <form onSubmit={submit} className="mt-4">
          {items.map((it,idx)=>(
            <div key={idx} className="mb-4">
              <label className="block text-sm mb-1">{it.label}</label>
              <input type="range" min="0" max="100" value={it.value} onChange={(e)=>handleChange(idx,e.target.value)} />
              <div className="text-sm mt-1">Nilai: {it.value}</div>
            </div>
          ))}
          <div className="flex justify-end mt-6">
            <button type="submit" className="bg-unizar text-white px-4 py-2 rounded">Simpan & Lihat Hasil</button>
          </div>
        </form>
      </div>
    </div>
